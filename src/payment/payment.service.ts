...
import { normalizePhoneNumber } from "../utils/normalizePhoneNumber";
import { getAccessToken, generatePassword } from "../utils/helper";
import { creditSellerWalletService } from "../wallet/wallet.service";
...
export const handleMpesaCallback = async (
  ...
  try {
    ...
    for (const it of items) {
      ...
      if (!earningsByShop[it.shopId]) earningsByShop[it.shopId] = 0;
      earningsByShop[it.shopId] += lineTotal;
    }

    const COMMISSION_PERCENT = Number(process.env.MARKETPLACE_COMMISSION) || 10;

    for (const shopIdStr of Object.keys(earningsByShop)) {
      const shopId = Number(shopIdStr);
      const shopTotal = earningsByShop[shopId];
      const commission = (shopTotal * COMMISSION_PERCENT) / 100;
      const sellerAmount = Number((shopTotal - commission).toFixed(2));

      const shopRecord = await db.query.shops.findFirst({ where: eq(shops.id, shopId) });
      ...
      await creditSellerWalletService({
        ...
        amount: sellerAmount,
        orderId,
        note: `Order ${orderId} escrow credit`,
      });
    ...
    console.error("Escrow split error:", err);
  }

  console.log(`Order ${orderId} successfully paid & escrowed`);
};
