...
import db from "../Drizzle/db";
import { flashSales, products } from "../Drizzle/schema";
import { lt, gt, and, eq, sql } from "drizzle-orm";
import { getIo } from "../socket";
...
export const updateFlashSaleStatuses = async () => {
  ...
  //Emit socket events for started/ended sales 
  allSales.forEach((sale) => {
    const updated = updatedSales.find((u) => u.id === sale.id);
    if (!updated) return;
  ...
  //Delete all ended sales & restore product flags 
  const deleted = await db
    .delete(flashSales)
    .where(sql`${flashSales.endTime} < NOW()`)
    .returning({ productId: flashSales.productId });

  // Mark affected products as no longer on flash sale
  ...
  console.log(
    `[FlashSaleScheduler] Status update completed:
    - Started/Ended events emitted
    - Deleted ${deleted.length} ended flash sales
    - Restored ${deleted.length} products
